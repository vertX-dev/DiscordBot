...
import { voteWeight } from './levels.js';
...
async function pollVote(interaction) {
  ...
  const poll = await getPoll(interaction.guild.id, pollId);
  if (!poll || !poll.open) {
    return interaction.reply({ ephemeral: true, content: 'This poll is closed.' });
  }
  const weight = poll.weighted ? await voteWeight(interaction.guild, interaction.user.id) : 1;
  const updated = await updatePoll(interaction.guild.id, pollId, (p) => {
    recordVote(p, Number(idxStr), interaction.user.id);
    if (p.weighted) { p.weights = p.weights || {}; p.weights[interaction.user.id] = weight; }
  });
...
async function pollEnd(interaction) {
  ...
  const closed = await updatePoll(interaction.guild.id, pollId, (p) => { p.open = false; });
...
async function addonVote(interaction) {
  const [, pollId] = interaction.customId.split(':');
  const poll = await getAddonPoll(interaction.guild.id, pollId);
  if (!poll || !poll.open) {
    return interaction.reply({ ephemeral: true, content: 'This poll is closed.' });
  }
  const picks = interaction.values;
  const weight = poll.weighted ? await voteWeight(interaction.guild, interaction.user.id) : 1;
  const updated = await updateAddonPoll(interaction.guild.id, pollId, (p) => {
    p.votes[interaction.user.id] = picks;
    if (p.weighted) { p.weights = p.weights || {}; p.weights[interaction.user.id] = weight; }
  });
