...
async function pollVote(interaction) {
  ...
  const updated = await updatePoll(interaction.guild.id, pollId, (p) => recordVote(p, Number(idxStr), interaction.user.id));
...
async function addonVote(interaction) {
  ...
  const updated = await updateAddonPoll(interaction.guild.id, pollId, (p) => { p.votes[interaction.user.id] = picks; });
