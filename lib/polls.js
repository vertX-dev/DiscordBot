...
// Weight a single voter — 1 normally, or their captured leveling weight when the
// poll is weighted.
function voterWeight(poll, uid) {
  return poll.weighted ? (poll.weights?.[uid] ?? 1) : 1;
}

// --- Generic poll (single choice, button voting) ---------------------------
export function buildPollEmbed(poll) {
  const optionTotal = (o) => o.votes.reduce((a, u) => a + voterWeight(poll, u), 0);
  const total = poll.options.reduce((a, o) => a + optionTotal(o), 0);
  const voters = poll.options.reduce((a, o) => a + o.votes.length, 0);
  const lines = poll.options.map((o, i) => {
    const count = optionTotal(o);
    const pct = total ? Math.round((count / total) * 100) : 0;
    return `**${i + 1}. ${o.label}**\n${bar(pct)}  ${count} · ${pct}%`;
  });
  const mode = poll.weighted ? 'weighted by level' : 'one vote each';
  return new EmbedBuilder()
    .setTitle(`📊 ${poll.question}`)
    .setDescription(lines.join('\n\n'))
    .setColor(poll.open ? 0x5865f2 : 0x95a5a6)
    .setFooter({ text: poll.open ? `${voters} voter(s) · ${mode}` : `Closed · ${voters} voter(s)` });
}
...
export function tallyAddons(poll) {
  ...
  for (const [uid, picks] of Object.entries(poll.votes)) {
    const w = poll.weighted ? (poll.weights?.[uid] ?? 1) : 1;
    for (const a of picks) if (a in counts) counts[a] += w;
  }
...
export function buildAddonEmbed(poll) {
  ...
  return new EmbedBuilder()
    ...
    .setFooter({
      text: poll.open
        ? `${voters} voter(s) · pick up to ${poll.maxVotes}${poll.weighted ? ' · weighted by level' : ''} · open`
